...
std::string findDecimal(const int& numerator, const int& denominator)
{
    ...
    while(remainder != 0)
    {
        if(remainders.find(remainder) != remainders.end())
        {
            ...
            result += ")";
            std::cout << "Number period length: " << result.length() - intLength - 1 - remainders[remainder] << "\n";
            break;
        }
...
int main()
{
    ...
    std::string result = findDecimal(numerator, denominator);
